...
def choice(request):
    ...
    if not q:
        ...
        if q == "VIDEO":
            documents = Document.objects.filter(user_id=user_id).filter(
            ...
            documents = Document.objects.filter(user_id=user_id).filter(Q(docfile__iregex=".ZIP"))

        else:
            documents = Document.objects.filter(user_id=user_id).all()
